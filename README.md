...
This repo presents my NixOS/Home-Manager configuration files from 2 hosts:

- a desktop (AMD CPU and iGPU, Nvidia eGPU)
- a laptop (Ubuntu, Home-Manager for the main user)
...
associated constraints (different CPUs, GPUs, headless, etc.), I took great care
of splitting what is OS-related from what is user-related, so as to have as many
things as possible ported to the Ubuntu machine.
...
- NixOS modules (like KVM, Steam) are defined in `modules/` and imported by
  `programs.nix`
- Home-Manager base configuration is common to all machines. A proper
  Home-Manager installation is generated by `lib/mkHome`
